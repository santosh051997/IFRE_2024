...
class FollowerNode(Node):
    ...
    def control_loop(self):
        ...
        #     linear_velocity = min(linear_velocity, 0.2)

        # if linear_velocity < -0.2:
        #     linear_velocity = max(linear_velocity, -0.2)

        # if angular_velocity > 0.07:
        #     angular_velocity = min(angular_velocity, 0.07)

        # if angular_velocity < -0.07:
        #     angular_velocity = max(angular_velocity, -0.07)
        
        self.get_logger().info("Linear Velocity: %f  Angular Velocity: %f " % (linear_velocity, angular_velocity))
